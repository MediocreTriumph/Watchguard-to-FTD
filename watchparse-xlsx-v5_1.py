...
def build_address_value_lookup(config):
    """
    Build a lookup that maps address object names to their actual values.
    Returns dict: name -> human-readable value string.

    Resolution:
      Host name      -> "10.0.1.5"
      Network name   -> "10.0.1.0/255.255.255.0"
      Range name     -> "10.0.1.1 - 10.0.1.50"
      FQDN name      -> "example.com"
      "Any"          -> "Any"
      Unresolved     -> "⚠ <original_name>" (passthrough with warning marker)
    """
    lookup = {}
    for h in config["addresses"]["hosts"]:
        lookup[h["name"]] = h["ip"]
    for n in config["addresses"]["networks"]:
        lookup[n["name"]] = f"{n['network']}/{n['mask']}"
    for r in config["addresses"]["ranges"]:
        lookup[r["name"]] = f"{r['start']} - {r['end']}"
    for f in config["addresses"]["fqdns"]:
        lookup[f["name"]] = f["fqdn"]
    lookup["Any"] = "Any"
    return lookup


def resolve_members_to_values(member_names, value_lookup):
    """Resolve a list of address object names to their actual values."""
    resolved = []
    for name in member_names:
        if name in value_lookup:
            resolved.append(value_lookup[name])
        else:
...
def write_workbook(config, issues, output_file):
    ...
    # --- Policies (Resolved) ---
    # Same as Policies but source/destination members resolved to actual IP/network/FQDN values
    value_lookup = build_address_value_lookup(config)
    ws_pol_res = wb.create_sheet("Policies (Resolved)")
    ws_pol_res.sheet_properties.tabColor = "7030A0"

    resolved_rows = []
    for p in config["policies"]:
        src_values = resolve_members_to_values(p["source_members"], value_lookup)
        dst_values = resolve_members_to_values(p["destination_members"], value_lookup)
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: ./watchparse-xlsx-v5.py <config.xml>")
        sys.exit(1)
    ...
    print(f"✓ Output written to {xlsx_file}")
    print(f"\n{'='*60}")
